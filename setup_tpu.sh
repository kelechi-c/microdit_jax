...
sudo apt install -y -qq golang neofetch zsh byobu btop && sudo snap install btop && sudo apt install -y -qq software-properties-common && sudo add-apt-repository -y ppa:deadsnakes/ppa && sudo apt-get install -y -qq python3.12
